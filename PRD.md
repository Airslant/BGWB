...
- 白板整理工具：
  - 左侧浮动工具栏提供：选择、添加桌游、文字、模版、分区、矩形、直线、箭头。
  - 模版为二级列表，第一版包含：便签、从夯到拉、象限、Top N、表格；选择模版后在画布点击生成对应标注对象。
  - 模版对象不是基础线条和文本的组合，而是可整体选择、移动和缩放的独立组件；不显示外框式容器。
  - 象限模版为单个整体对象，内部包含 X/Y 双箭头十字轴和四个角落文字区；只能整体缩放，并统一控制颜色、线宽和字号。
  - 从夯到拉模版为单个整体对象，左侧等级文字固定不可编辑，右侧列表区域可通过专用手柄单独横向延长；默认尺寸下每个等级行的右侧列表高度应能容纳一张默认桌游卡片。
  - Top N 模版为单个表格对象，每个格子用于摆放一张桌游卡片；新建时单格尺寸略大于默认桌游卡片；样式控制条支持设置 Top 数量、正序/倒序、行数和列数，Top 数量最高为 100。
  - 表格模版为单个整体对象，每个格子用于摆放或整理桌游卡片；样式控制条只支持设置行数和列数，不提供字号调节。
...
- 管理后台：
  - 用户管理支持按邮箱/昵称检索，展示角色、状态、注册时间、白板数、白板上限和卡片数；支持禁用/启用普通用户，也支持调整单个用户的白板数量上限。
...
- 账号白板数量上限：
  - 单个账号默认最多创建 20 个白板。
  - 达到上限后前端禁用新建入口，服务端 `POST /api/boards` 也会拒绝创建。
  - 管理员可在后台用户管理中为单个用户调整上限，调整后立即影响后续新建白板。
- 收藏状态第一版固定为：`无`、`拥有`、`想玩`、`玩过`、`心愿单`、`已出掉`。单个桌游可选一个状态，新添加桌游默认 `无`；`无` 作为空状态，不在封面上显示状态角标。
- 基础信息第一版字段：
...
- `PATCH /api/admin/users/:userId`：启用/禁用用户，或修改该用户白板数量上限。
...
- 数据结构核心：
  - `User`: `id`, `nickname`, `email`, `maxBoards`, `createdAt`
  - `AdminUser`: `id`, `nickname`, `email`, `maxBoards`, `role`, `disabledAt`, `disabledReason`, `createdAt`, `updatedAt`
...
- 单个账号达到白板上限后不能继续新建白板；管理员调整该账号上限后，限制按新值生效。
- 退出登录后无法访问白板列表和编辑接口。
...
- 创建文字、模版、分区、矩形、直线、箭头后，等待自动保存完成，刷新页面仍保留位置、尺寸、文本和样式。
- 公开分享页能看到所有整理工具对象，但不能编辑、删除、拖动或调样式。
- 拖动整理对象接近其它整理对象的边缘或中心时，会自动磁吸并显示对齐参考线。
- 工具快捷键在编辑态白板中可切换对应工具；只读分享页不显示工具快捷键入口，也不能通过快捷键创建或编辑对象。
- 小地图随卡片、整理组件和视野变化更新；点击或拖动小地图后主画布视野移动到对应区域。
- `Esc`、`Delete` / `Backspace` 在输入框聚焦时不误触画布操作；未聚焦时可退出工具、关闭弹层、取消选中或删除选中整理对象。
